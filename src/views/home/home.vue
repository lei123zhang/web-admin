			...
			<Col span="7">
				<Card>
					...
					<div class="cardBody">
						...
						<p>总交易额 <span>{{ otcChart.borderInfo.money }}</span></p>
						<p>总手续费 <span class="fee">{{ otcChart.borderInfo.fee }}</span></p>
					...
					<div class="cardFooterWrapper">
						<Row class="cardFooter">
							<Col span="11" offset="1">
								<p>昨日交易量 <span>{{ otcChart.borderInfo.yesterdayAmount }}</span></p>
							</Col>
							<Col span="11" offset="1">
								<p>昨日手续费 <span>{{ otcChart.borderInfo.yesterdayFee }}</span></p>
							</Col>
						</Row>
					</div>
				</Card>
			...
			<Col span="7">
				<Card>
					...
					<div class="cardBody">
						...
						<p style="line-height: 60px">总手续费 <span class="fee">{{ exchangeChart.boardInfo.fee }}</span></p>
						<div class="selectWrapper">
							<Select v-model="exchangeChart.board.unit" size="small">
					...
					<div class="cardFooterWrapper">
						<Row class="cardFooter" justify="space-around">
							<Col span="11">
								<p>昨日交易量 <span>{{ exchangeChart.boardInfo.yesterdayAmount }}</span></p>
							</Col>
							<Col span="11">
								<p>昨日手续费 <span>{{ exchangeChart.boardInfo.yesterdayFee }}</span></p>
							</Col>
						</Row>
					</div>
				</Card>
...
import { memberBoard, memberChart, otcChart, coinChart, otcBoard, coinBoard,
		...
		methods: {
			...
			exchangeManualChart(sourceData, obj) {
				const chart = new G2.Chart({
						...
						chart.interval().position(`时间*${obj.type}`).color('币种').adjust('dodge');
			...
			exchangeChartFn(searchObj) {
				coinChart(searchObj)
					.then(res => {
					if(!res.code) {
						this.exchangeManualChart(res.data, { ref: this.$refs.exchangeNumChart, type: '交易量' })
						this.exchangeManualChart(res.data, { ref: this.$refs.exchangeMoneyChart, type: '交易额' })
					}else this.$Message.error(res.message);
				})
				.catch(err => console.log(err))
			},
